...
echarts.util.each(days, function (day, idx) {
    week_option.title.push({
        textBaseline: 'middle',
        top: (idx + 0.5) * 100 / 7 + '%',
        text: day

    });
    week_option.singleAxis.push({
        left:70,
        type: 'category',
        boundaryGap: false,
        data: hours,
...
echarts.util.each(data, function (dataItem) {
    week_option.series[dataItem[0]].data.push([dataItem[1], dataItem[2]]);
});
...
window.setInterval(function () {
    $.get(s_path+"/plot",function (data1) {
        ...
        echarts.util.each(data1, function (dataItem) {
            ...
            week_option.series[dataItem[0]].data=[];
        });
        echarts.util.each(data1, function (dataItem) {
            //console.log(week_option.series[dataItem[0]].data);
            week_option.series[dataItem[0]].data.push([dataItem[1], dataItem[2]]);
        });
        week_Chart.setOption(week_option,true);
    });
},3000);